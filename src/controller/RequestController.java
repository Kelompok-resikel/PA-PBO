...
public void loadRequestsDataKru(DefaultTableModel tableModel) {

    // Query SQL untuk mengambil data yang akan ditampilkan
    String query = "SELECT r.id_request, u.nama_lengkap, p.kontak, p.domisili, s.jenis_sampah, r.jam_masuk, r.jam_jemput, " +
               "r.status, r.estimasi, r.kru_penjemput " +
               "FROM recycle.request r " +
               "JOIN recycle.sampah s ON r.sampah_id_sampah = s.id_sampah " +
               "JOIN recycle.users u ON r.users_id_user = u.id_user " +
               "JOIN recycle.pengguna p ON u.id_user = p.id_user";
    ...
    try (Connection connection = database.getNewConnection();
        ...
        try (ResultSet resultSet = statement.executeQuery()) {
            ...
            while (resultSet.next()) {
                Object[] rowData = {
                    resultSet.getInt("id_request"),         // ID Request
                    resultSet.getString("nama_lengkap"),    // Nama Lengkap
                    resultSet.getString("kontak"),          // Kontak
                    resultSet.getString("domisili"),        // Domisili
                    resultSet.getString("jenis_sampah"),    // Jenis Sampah
                    resultSet.getString("jam_masuk"),       // Jam Masuk
                    resultSet.getString("jam_jemput"),      // Jam Jemput
                    resultSet.getString("status"),          // Status
                    resultSet.getString("estimasi"),        // Estimasi
                    resultSet.getString("kru_penjemput")    // Kru Penjemput
                };
                // Tambahkan data ke model tabel
                tableModel.addRow(rowData);
            }
...
public void updateRequestKru(int requestId, String newStatus, String newJamJemput, String newEstimasi, String newKruPenjemput) {
    // Validasi status baru
    if (!newStatus.equals("PROGRESS") && !newStatus.equals("CANCEL") && !newStatus.equals("SUKSES")) {
        System.err.println("Status tidak valid. Hanya bisa: PROGRESS, CANCEL, SUKSES.");
        return;
    }

    // Query SQL untuk memperbarui data
    String query = "UPDATE recycle.request " +
                   "SET status = ?, jam_jemput = ?, estimasi = ?, kru_penjemput = ? " +
                   "WHERE id_request = ?";
    ...
    try (Connection connection = database.getNewConnection();
        ...
        statement.setString(1, newStatus);
        statement.setString(2, newJamJemput);
        statement.setString(3, newEstimasi);
        statement.setString(4, newKruPenjemput);
        ...
        if (rowsAffected > 0) {
            ...
            System.err.println("Gagal memperbarui data. ID Request mungkin tidak ditemukan.");
        }
    } catch (SQLException e) {
...
public void deleteRequestKru(int requestId) {
    // Query SQL untuk menghapus data berdasarkan ID Request
    String query = "DELETE FROM recycle.request WHERE id_request = ?";
    ...
    try (Connection connection = database.getNewConnection();
        ...
        if (rowsAffected > 0) {
            ...
            System.err.println("Gagal menghapus data. ID Request mungkin tidak ditemukan.");
        }
    } catch (SQLException e) {
